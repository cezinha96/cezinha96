...
const prefix = '='
const botName = "Cezinha-BOT"
const ownerNumber = "5574999510904"
const ownerName = "Cezinha"
const cdd = 480000 // 8 minutos
const ammOff = "off"
const blocked = []

//_LISTA DE CRS
const cr = "Cezinha-BOT v12.4"
...
async function starts() {
	const Cezinha = new WAConnection()
	Cezinha.logger.level = 'warn'
	console.log(banner.string)
	Cezinha.on('qr', () => {
		console.log(color('👆'), color(' Escanear o código acima para iniciar o Cezinha-BOT!'))
	})

	fs.existsSync('./Cezinha.json') && Cezinha.loadAuthInfo('./Cezinha.json')
	Cezinha.on('connecting', () => {
		start('2', 'Conectando o Cezinha-BOT...')
	})
	Cezinha.on('open', () => {
		success('2', 'Cezinha-BOT conectado!!!')
	})
	await Cezinha.connect({timeoutMs: 30*1000})
        fs.writeFileSync('./Cezinha.json', JSON.stringify(Cezinha.base64EncodedAuthInfo(), null, '\t'))

//_FUCTION DE BOAS-VINDAS
Cezinha.on('group-participants-update', async (anu) => {
		if (!welkom.includes(anu.jid)) return
		try {
			const mdata = await Cezinha.groupMetadata(anu.jid)
			console.log(anu)
			if (anu.action == 'add') {
				num = anu.participants[0]
				try {
					ppimg = await Cezinha.getProfilePicture(`${anu.participants[0].split('@')[0]}@c.us`)
				} catch {
					ppimg = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
				}
				teks = `Olá @${num.split('@')[0]}\nSeja bem vindo(a) ao grupo: ${mdata.subject}`
				let buff = await getBuffer(ppimg)
				Cezinha.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			} else if (anu.action == 'remove') {
				num = anu.participants[0]
				try {
					ppimg = await Cezinha.getProfilePicture(`${num.split('@')[0]}@c.us`)
				} catch {
					ppimg = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
				}
      ...
      Cezinha.on('chat-update', async (mek) => {
		try {
            ...
            const totalchat = await Cezinha.chats.all()
			const botNumber = Cezinha.user.jid
			const ownerNumber = [`557499510904@s.whatsapp.net`]
			const ownerNumberB = []
			//const ban = [`557499510904@s.whatsapp.net`]
			const isGroup = from.endsWith('@g.us')
			const sender = isGroup ? mek.participant : mek.key.remoteJid
			const groupMetadata = isGroup ? await Cezinha.groupMetadata(from) : ''
			...
			let pushname = Cezinha.contacts[sender] != undefined ? Cezinha.contacts[sender].vname || Cezinha.contacts[sender].notify: undefined
           ...
           mess = {
			...
			const reply = (tej) => {
				Cezinha.sendMessage(from, tej, text, {quoted:mek})
			}
			const sendMess = (teks) => {
				Cezinha.sendMessage(from, teks, text)
			}
            const sendImage = (tis) => {
		        Cezinha.sendMessage(from, tis, image, {quoted:mek})
            }
			const mentions = (ops, memberr, sender, id) => {
				(id == null || id == undefined || id == false) ? Cezinha.sendMessage(from, ops.trim(), extendedText, {contextInfo: {"mentionedJid": memberr}}) : Cezinha.sendMessage(from, ops.trim(), extendedText, {quoted: mek, contextInfo: {"mentionedJid": memberr}})
			}
			const costum = (pesan, tipe, target, target2) => {
                 Cezinha.sendMessage(from, pesan, tipe, {quoted: {key: {fromMe: false, participant: `${target}`, ...(from ? {remoteJid: from}: {})}, message: {conversation: `${target2}` }}})
            }
             const sendPtt = (teks) => {
                 Cezinha.sendMessage(from, audio, mp3, {quoted: mek })
            }
...
Cezinha.sendMessage(from, lvup, text, {quoted: mek})
//Cezinha.sendMessage(from, lvup, text, {quoted: mek, contextInfo: {mentionedJid: [sender]}}
                }
            } catch (err) {
                console.error(err)
            }
...
////FINALIZAR TTT AUTOMATICAMENTE By: Resen
			if (tttset.tttstatus == "off" && tttset.autoEndTime == "on") {
				...
				if (isLevelingOn) {
					...
					Cezinha.sendMessage(tttset.local,`❌ O TEMPO DE JOGO EXPIROU ❌\n\n➣  PUNIÇÃO: ${randomEndTTTXP} XP 🔮`, text, {quoted: tttset.mentionPlayer})
				} else {
					Cezinha.sendMessage(tttset.local,`❌ O TEMPO DE JOGO EXPIROU ❌`, text, {quoted: tttset.mentionPlayer})
				}
...
//_"Es hora de mimir" By: Resen
			...
			if (horaAtual == "10:10:00" && ammOff == "off" && isGroup) {
				...
				Cezinha.sendMessage(from, sonica, MessageType.audio, {mimetype: 'audio/mp4', ptt:true})
				Cezinha.sendMessage(from, sonico, sticker)
				ammOff = "on"
...
authorname = Cezinha.contacts[from] != undefined ? Cezinha.contacts[from].vname || Cezinha.contacts[from].notify : undefined	
if (authorname != undefined) { } else { authorname = groupName }	

//_FUCTION NOME DOS STICKERS
                function addMetadata(packname, author) {	
				if (!packname) packname = 'Cezinha-BOT'; if (!author) author = 'Bot';	
...
Cezinha.sendMessage(from, buf, audio, {mimetype: 'audio/mp4', quoted: mek, ptt: true})
}
if ((budy === "fdp") ||
(budy === "Fdp") ||
(budy === "FDP")) {
Cezinha.updatePresence(from, Presence.composing)
reply("teu pai")
}
if ((budy === "oi bot") ||
(budy === "Oi bot")) {
...
Cezinha.sendMessage(from, cleitu, sticker, {quoted: mek})
}
if ((budy === "Edmilson") ||
(budy === "edmilson")) {
...
await axios.get(`https://nekos.life/api/v2/img/pussy_jpg`).then((res) => {
	Cezinha.sendMessage(from, '[🦧] ESPERE ESTA PROCESSANDO...', MessageType.text)
   // let cpf = ${res.data.url}
    Cezinha.sendMessage(from, res.data.url,MessageType.image);
})
}*/
...
axios.get(`http://geradorapp.com/api/v1/cpf/generate?token=40849779ec68f8351995def08ff1e2fa`).then((res) => {
    ...
    Cezinha.sendMessage(from, cpf,MessageType.text);
})
}

if (budy.match('Cezinha')) {
result = fs.readFileSync(`./teste/mask.webp`)
Cezinha.sendMessage(from, result, sticker, {quoted: mek })
} else if (budy.match('Cezinha')) {
result = fs.readFileSync(`./teste/mask.webp`)
Cezinha.sendMessage(from, result, sticker, {quoted: mek })
} else if (budy.match('Cezinha')) {
result = fs.readFileSync(`./teste/mask.webp`)
...
switch(anun) {
case 'dança':
buf = fs.readFileSync(`./teste/Cezinha.webp`)
Cezinha.sendMessage(from, buf, sticker, {quoted: mek})
break
...
case 'esh':
Cezinha.sendMessage(from, esh, text, {quoted: mek})
break
...
Cezinha.sendMessage(from, Menu, text, { quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "caption": "Cezinha-BOT v12.4", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('teste/bot.jpg')}}}})
...
Cezinha.sendMessage(from, `https://github.com/CezinhaH/Cezinha-BOT` , text, {quoted: mek})
break
...
Cezinha.sendMessage(from, anu, image, {quoted: mek})
break
...
Cezinha.sendMessage(from, buf, video, {quoted: mek})
break

case 'ban':
if (!isOwner) return reply(mess.only.ownerB)
addBan(body.slice(4))
Cezinha.sendMessage(from, `você foi banido ${body.slice(4)}@c.us`, text)
break
...
case 'block':
				 ...
				 Cezinha.chatRead (from)
					...
					Cezinha.blockUser (`${body.slice(7)}@c.us`, "add")
					Cezinha.sendMessage(from, `perintah Diterima, memblokir ${body.slice(7)}@c.us`, text)
					Cezinha.sendMessage(from, { quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "caption": "_「 の ＭｅＩｋｙ あ」_", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('image/odc.jpeg')}}}})
...
break
				    ...
				    Cezinha.blockUser (`${body.slice(9)}@c.us`, "remove")
					Cezinha.sendMessage(from, `Perintah Diterima, membuka ${body.slice(9)}@c.us`, text)
					Cezinha.sendMessage(from, { quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "caption": "_「 の ＭｅＩｋｙ あ」_", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('image/odc.jpeg')}}}})
...
Cezinha.sendMessage(from, sapa, {quoted: mek})
break
...
const horatt = moment.tz('America/Sao_Paulo').format('HH:mm')
			...
			if (/*horaAtual == '08:42' && */ammOff == "off" && isGroup) {
				...
				Cezinha.sendMessage(_.jid, aMimirMp3, MessageType.audio, {mimetype: 'audio/mp4', ptt:true})
				Cezinha.sendMessage(_.jid, aMimirSt, sticker)
				}
				ammOff = "on"
...
case 'jsonfiles':
						...
						Cezinha.sendMessage(from, daftarfile, document, {mimetype: 'json', filename: 'daftar.json'})
						const diariofile = fs.readFileSync('./database/json/diario.json')
						Cezinha.sendMessage(from, diariofile, document, {mimetype: 'json', filename: 'diario.json'})
						const levelfile = fs.readFileSync('./database/json/level.json')
						Cezinha.sendMessage(from, levelfile, document, {mimetype: 'json', filename: 'level.json'})
						const levelingfile = fs.readFileSync('./database/json/leveling.json')
						Cezinha.sendMessage(from, levelingfile, document, {mimetype: 'json', filename: 'leveling.json'})
						const nsfwfile = fs.readFileSync('./database/json/nsfw.json')
						Cezinha.sendMessage(from, nsfwfile, document, {mimetype: 'json', filename: 'nsfw.json'})
						const simifile = fs.readFileSync('./database/json/simi.json')
						Cezinha.sendMessage(from, simifile, document, {mimetype: 'json', filename: 'simi.json'})
						const tictactoefile = fs.readFileSync('./database/ttt/tictactoe.json')
						Cezinha.sendMessage(from, tictactoefile, document, {mimetype: 'json', filename: 'tictactoe.json'})
						const userfile = fs.readFileSync('./database/json/user.json')
						Cezinha.sendMessage(from, userfile, document, {mimetype: 'json', filename: 'user.json'})
						const welkomfile = fs.readFileSync('./database/json/welkom.json')
						Cezinha.sendMessage(from, welkomfile, document, { mimetype: 'json', filename: 'welkom.json'})
					break
...
cbuff = `djsjsj`
Cezinha.sendMessage(from, cbuff, image, {quoted: freply, caption: `Silahkan jawab soal berikut dengan benar\nwaktu: 30 detik`})
reply('30 segs')
sleep(30000)
reply('20 segs')
sleep(20000)
...
Cezinha.sendMessage(from, attp2, sticker, {quoted: mek})
break
...
case 'pão':
if (isOwner) return reply('Oi Cezinha')
else {
...
Cezinha.sendMessage(from, feg, MessageType.image)
break
...
║╰───────────
┗━━━━━━━━━━━━┛`     
Cezinha.sendMessage(from, levela, text, {quoted: mek, contextInfo: {mentionedJid: [sender]}})
break

//_ENVIA O VCARD
case 'Cezinha':
const sentMsg  = await Cezinha.sendMessage(from, {displayname: "Cezinha🧙‍♂️", vcard: vcard}, MessageType.contact)
break
                    case 'leaderboard':
				case 'lb':
                ...
                try {
                    ...
                    Cezinha.sendMessage(from, leaderboardlvl, text, {quoted: mek})
                } catch (err) {
                    console.error(err)
...
//Cezinha.groupRemove(from, mentioned)
//Cezinha.groupAdd(from, [num])
				  case 'amongus':
				msgFilter.isFiltered(from)
					if (!isGroup) return reply(mess.only.group)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return 
...
//var group = await Cezinha.groupMetadata(from)
case 'testime':
					setTimeout( () => {
					Cezinha.sendMessage(from, 'O tempo acabou', text) 
					}, 10000)
					setTimeout( () => {
					Cezinha.sendMessage(from, '5 segundos restantes', text)
					}, 5000)
					setTimeout( () => {
					Cezinha.sendMessage(from, '10 segundos restantes', text)
					}, 0)
					break
...
if (!isOwner) return reply(mess.only.ownerB)
                   ...
                   reply('A mimir...')
	               exec(pm2, (err, stdout) => {
		           if(err) return Cezinha.sendMessage(from, "Comando inexistente", text, {quoted: mek})

		           if (stdout) {
			       Cezinha.sendMessage(from, stdout, text, {quoted: mek})
} 
                   })
                  break
...
Cezinha.sendMessage(from, teks, text, {quoted: mek})
...
Cezinha.sendMessage(from, buffer, image, {quoted: mek})
teks = '𝗣𝗹𝗮𝘆 𝘀𝘁𝗼𝗿𝗲\n'
...
dung = (anu.sticker)
Cezinha.sendMessage(from, buffer, image, {quoted: mek, caption: teks})
var tes2 =  dung[Math.floor(Math.random() * dung.length)];
exec(`wget ${tes2} -O ${ranp} && ffmpeg -i ${ranp} -vcodec adminwebp -filter:v fps=fps=15 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
if (err) return reply('Error cok')
fs.unlinkSync(ranp)
...
Cezinha.sendMessage(from, buffer, image, {quoted: mek, caption: teks.trim()})
break
...
case 'slapimage': 
try {
profil = await Cezinha.getProfilePicture(`${sender.split('@')[0]}@s.whatsapp.net`)
} catch {
...
Cezinha.sendMessage(from, buffer, image, {quoted: mek, caption: 'Nih'})
break
             ...
             case 'waifukawai':
					...
					Cezinha.sendMessage(from, buf, image, {quoted: mek,caption: "KAWAII!!"})
					})})
					} catch (e) {
			...
			case 'kemonomimi':
	   			...
	   			reply(mess.wait)
						axios.get(`https://nekos.life/api/v2/img/kemonomimi`).then((res)=>{
						imageToBase64(res.data.url).then((response) => {var buf = Buffer.from(response, 'base64');
					Cezinha.sendMessage(from, buf, image, {quoted: mek,caption: "ONII CHAN BAKA!!"})
					})})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply('тЭМ *ERROR* тЭМ')
		...
		case 'kuni':
					...
					reply(mess.wait)
						...
						imageToBase64(res.data.url).then((response) => {var buf = Buffer.from(response, 'base64');
					Cezinha.sendMessage(from, buf, image, {quoted: mek,caption: "*INGAT ADA TUHAN*!!"})
					})})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply('тЭМ *ERROR* тЭМ')
				...
				if (isBanned) return reply(mess.erro.ban)
					...
					Cezinha.sendMessage(from, nye, image, { caption: 'kyaa one chan!!', quoted: mek })
					await limitAdd(sender)
					break 
            ...
            media = await Cezinha.downloadAndSaveMediaMessage(encmedia)
            ran = getRandom('.mp4')
            exec(`ffmpeg -i ${media} -vf reverse -af areverse ${ran}`, (err) => {
            fs.unlinkSync(media)
            if (err) return reply(`Err: ${err}`)
            buffer453 = fs.readFileSync(ran)
            Cezinha.sendMessage(from, buffer453, video, { mimetype: 'video/mp4', quoted: mek })
            fs.unlinkSync(ran)
            })
            break
                case 'fastvid':
...
case 'togif':
				case 'tomedia':
                                        ...
                                        const media = await Cezinha.downloadAndSaveMediaMessage(encmedia)
                                        const upload = await uploadimg(media, Date.now() + '.webp')
                                        console.log(upload)
                                        const rume = await axios.get(`http://nzcha-apii.herokuapp.com/webp-to-mp4?url=${upload.result.image}`)
                                        const buff = await getBuffer(rume.data.result)
                                        console.log(rume.data)
                                        Cezinha.sendMessage(from, buff, video, { mimetype: Mimetype.gif, caption: 'Nih', quoted: mek})
                                }
                                break
		...
		case 'neko2':
				...
				reply(mess.wait)	
					axios.get(`https://nekos.life/api/v2/img/meow`).then((res)=>{
						imageToBase64(res.data.url).then((response) => {var buf = Buffer.from(response, 'base64');
				Cezinha.sendMessage(from, buf, image, {quoted: mek,caption: "NYANG!!"})
					})})
					} catch (e) {
				console.log(`Error :`, color(e,'red'))
						reply('тЭМ *ERROR* тЭМ')
      ...
      case 'holo':
					...
					reply(mess.wait)
						...
						imageToBase64(res.data.url).then((response) => {var buf = Buffer.from(response, 'base64');
					Cezinha.sendMessage(from, buf, image, {quoted: mek,caption: "NIH OM!!"})
					})})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply('тЭМ *ERROR* тЭМ')
				...
				Cezinha.sendMessage(from, nan, sticker, {quoted: mek})
				break
...
Cezinha.sendMessage(from, teks.trim(), extendedText, {quoted: mek, contextInfo: {"mentionedJid": premium}})
break
...
Cezinha.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', ptt:true, quoted: mek})
fs.unlinkSync(ran)
})
break
...
hah = fs.readFileSync(ran)
Cezinha.sendMessage(from, hah, audio, {mimetype: 'audio/mp3/ptt/mp4', ptt:true, quoted: mek})
fs.unlinkSync(ran)
})
break
...
Cezinha.sendMessage(from, buff, image, {quoted: mek, caption: '✏📕'})
break
...
Cezinha.sendMessage(from, randBokep, image, {quoted: mek, caption: randTeks})
break
...
const requirXp = 500 * (Math.pow(2, usLevel) - 1)
  try {
ppimg = await Cezinha.getProfilePicture(`${sender.split('@')[0]}@c.us`)
  } catch {
ppimg = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
  }
 const pf = 
...
`
  ...
  Cezinha.sendMessage(from, its, image, {quoted: mek, caption: pf, contextInfo: {mentionedJid: [sender]}})
...
case 'owner':
msgFilter.isFiltered(from)
Cezinhaft = fs.readFileSync('./teste/gojo.jpg')
Cezinha.sendMessage(from, Cezinhaft, image, {quoted: mek, caption: `☆━✪🕴  ∴₰Ⱦꪋℓo፝֯֟ ߷  🕴✪━☆\n\nChat do baiano: wa.me/+5574999510904`})
break
...
slo = await Cezinha.downloadAndSaveMediaMessage(low)
ran = getRandom('.mp3')
...
tup = await Cezinha.downloadAndSaveMediaMessage(pai)
ran = getRandom('.mp3')
...
gem = await Cezinha.downloadAndSaveMediaMessage(muk)
ran = getRandom('.mp3')
...
bas = await Cezinha.downloadAndSaveMediaMessage(ass)
ran = getRandom('.mp3')
...
msgFilter.isFiltered(from)
 ...
 `‣ Nome do bot: ${me.name}
  ...
  Cezinha.sendMessage(from, buffer, image, {caption: inf, contextInfo: {mentionedJid: [me.jid]}})
  break
...
try {
ppUrl = await Cezinha.getProfilePicture(from)
} catch {
...
` 
  ...
  ‣Descrição:
  ${desc ? desc : ''}`
Cezinha.sendMessage(from, buffer, image, {quoted: mek, caption: infogp})
break
...
Cezinha.sendMessage(from, pok, image, {quoted: mek, caption: `Achei isso sobre: ${args}`})
} catch {
...
msgFilter.isFiltered(from)
			    ...
			    let online = [...Object.keys(Cezinha.chats.get(ido).presences), Cezinha.user.jid]
			    Cezinha.sendMessage(from, 'Lista de usuários online:\n' + online.map(v => '- @' + v.replace(/@.+/, '')).join`\n`, text, { quoted: mek,
...
nimek = n[Math.floor(Math.random() * n.length)];
pok = await getBuffer(nimek)
Cezinha.sendMessage(from, pok, image, {
...
reply(mess.wait)
const medipp = await Cezinha.downloadAndSaveMediaMessage(ftgp)
await Cezinha.updateProfilePicture (from, medipp)
...
msgFilter.isFiltered(from)
           ...
           reply(mess.waitimg)
         owgi = await Cezinha.downloadAndSaveMediaMessage(ger)
       ...
       exec(`wget ${anu1} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
                                                ...
                                                Cezinha.sendMessage(from, nobg, sticker, {quoted: mek})
...
if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
 ...
 Cezinha.sendMessage(from, hehe, image, {quoted:mek})
} else {
...
if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
  ...
  reply(mess.waitimg)
  owgi = await Cezinha.downloadAndSaveMediaMessage(ted)
  tels = body.slice(7)
  anu = await imgbb("0c419be2e8bfc27eff00147b0c763418", owgi)
...
Cezinha.sendMessage(from, heh, image, {quoted:mek})
break
///_photoOXY
...
` INFORMAÇÕES DO CEP
  ...
  ‣ Estado: ${hehe.state}
  ‣ Cidade: ${hehe.city}`
Cezinha.sendMessage(from, ccg, text, {quoted:mek})
break
...
Cezinha.sendMessage(from, buffer, image, {quoted: mek, caption: 'nyan!'})
} catch (e) {
console.log(`Error :`, color(e,'red'))
...
Cezinha.sendMessage(from, buffer, image, {quoted: mek, caption: `${hehe.name}`})
} catch (e) {
console.log(`Error :`, color(e,'red'))
reply('❌ocorreu um erro❌')
}
...
Cezinha.sendMessage(from, buffer, text, {quoted: mek})
} catch (e) {
console.log(`Error :`, color(e,'red'))
reply('❌ocorreu um erro❌')
}
...
Cezinha.sendMessage(from, hah.url, image, {quoted:mek})
break
...
Cezinha.sendMessage(from, hehe, text, {quoted: mek})
break
...
case 'nekogif':
await Cezinha.sendMessage(from, { url: `http://api.lolhuman.xyz/api/random2/nsfw_neko_gif?apikey=847de7716f17a51eeba4235c`}, MessageType.video, { mimetype: Mimetype.gif, caption: "teste!" })
break
...
case 'ts':
Cezinha.sendMessage(from, bahasa(prefix, sender), text, {
  quoted: mek
})
break
...
contextInfo: {
  ...
  Cezinha.sendMessage(from, options, text, {
quoted: mek
  })
  break

case 'tl':
if (args.length < 1) return Cezinha.sendMessage(from, `digite da forma correta: ${prefix}tl texto/língua`, text, {
  quoted: mek
})
var pc = body.slice(3)
nomor = pc.split("/")[0];
...
Cezinha.updatePresence(from, Presence.composing)
if (!isGroup) return reply(mess.only.group)
if(args.length < 1) return Cezinha.reply('escreva algo como aviso')
aviso  = `Aviso de: @${sender.split("@")[0]}\n\nAviso: ${body.slice(7)}`
group = await Cezinha.groupMetadata(from);
...
await Cezinha.sendMessage(from, options, text)
break
...
Cezinha.groupUpdateSubject(idgrup, `${body.slice(9)}`)
Cezinha.sendMessage(from, 'o nome do grupo foi alterado', text, {
  quoted: mek
})
break
...
if (!isGroupAdmins) return reply(mess.only.admin)
if (!isBotGroupAdmins) return reply(mess.only.Badmin)
Cezinha.groupUpdateDescription(from, `${body.slice(9)}`)
Cezinha.sendMessage(from, 'a descrição do grupo atualizada', text, {
  quoted: mek
})
break
...
Cezinha.sendMessage(from, tagme, text)
break
...
reply('Procurando sua música...⏳')
  ...
  aanu = await fetchJson(`https://api-Cezinha.Cezinhah.repl.co/api/yta?url=${anu.result.source}`)
  infomp3 = 
`    MÚSICA ENCONTRADA
  ...
  setTimeout( () => {
  Cezinha.sendMessage(from, buffer, image, {quoted: mek, caption: infomp3})
  }, 1500)
  reply('Baixando e enviando sua música...')
  Cezinha.sendMessage(from, lagu, audio, {mimetype: 'audio/mp4', quoted: mek})
  break
...
`
  ...
  Cezinha.sendMessage(from, buffer, image, {
...
`    DADOS GERADOS
   ...
   ‣ Senha: ${anu.password}
   ‣ CEP: ${anu.zip_code}`
Cezinha.sendMessage(from, dadosf, text, {quoted: mek})
break
...
msgFilter.isFiltered(from)
  ...
  try {
  anu = await fetchJson(`https://api-Cezinha.Cezinhah.repl.co/api/yta?url=${play}`)
...
quoted: mek, caption: infomp3
  ...
  Cezinha.sendMessage(from, lagu, audio, {
...
quoted: mek, caption: infomp3
  ...
  Cezinha.sendMessage(from, lagu, video, {
...
//_CAPTURA O TEXTO NA IMAGEM
				case 'ocr':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						const com = await Cezinha.downloadAndSaveMediaMessage(ocrt)
						reply(mess.wait)
						...
						await ffmpeg(`./${media}`)
							...
							.on('end', function () {
								...
								exec(`webpmux -set exif ${addMetadata(pack, autor)} ${ran} -o ${ran}`, async (error) => {
									...
									Cezinha.sendMessage(from, fs.readFileSync(ran), sticker, {quoted: mek})
                      ...
                      if (!q.includes('|')) return reply(`Você deve digitar o nome do pacote e criador dessa forma:\nComando: ${prefix}fign nome do pack|nome do autor\nExemplo: ${prefix}fign Pack nagatoro1|Cezinha`)
						...
						await removeBackgroundFromImageFile({path: media, apiKey: keyrmbg, size: 'auto', type: 'auto', ranp}).then(res => {
							...
							exec(`ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${ranw}`, (err) => {
								fs.unlinkSync(ranp)
								if (err) return reply(mess.error.stick)
								exec(`webpmux -set exif ${addMetadata('Cezinha-BOT', authorname)} ${ranw} -o ${ranw}`, async (error) => {
									if (error) return reply(mess.error.stick)
									Cezinha.sendMessage(from, fs.readFileSync(ranw), sticker, {quoted: mek})
                ...
                case 'figurinha':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('end', function () {
								console.log('Finish')
								exec(`webpmux -set exif ${addMetadata('Cezinha-BOT', authorname)} ${ran} -o ${ran}`, async (error) => {
...
case 'stk':
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('error', function (err) {
								console.log(`Error : ${err}`)
                                exec(`webpmux -set exif ${addMetadata('Cezinha-BOT', authorname)} ${rano} -o ${rano}`, async (error) => {
								fs.unlinkSync(media)
								reply(mess.error.stick)
							})
                      })
							...
							fs.unlinkSync(media)
						buffer = fs.readFileSync(rano)
						Cezinha.sendMessage(from, buffer, sticker, {quoted: mek})
			...
			case 'gets':
				...
				Cezinha.sendMessage(from, result, sticker, selepbot)
				} catch {
			...
			case 'liststik':
				...
				Cezinha.sendMessage(from, lt.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": setiker } })
				break
			...
			case 'totaluser':
				...
				Cezinha.sendMessage(from, ts.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": _registered} })
				break
			...
			case 'addstik':
				...
				delb = await Cezinha.downloadMediaMessage(boij)
				setiker.push(`${svst}`)
				fs.writeFileSync(`./temp/stick/${svst}.webp`, delb)
				fs.writeFileSync('./temp/stik.json', JSON.stringify(setiker))
				Cezinha.sendMessage(from, `Sukses Menambahkan Sticker\nCek dengan cara ${prefix}liststik`, MessageType.text, {quoted: mek})
			...
			case 'addvn':
				...
				Cezinha.sendMessage(from, `Sukses Menambahkan Audio\nCek dengan cara ${prefix}listvn`, MessageType.text, {quoted: mek})
			...
			case 'getvn':
				...
				Cezinha.sendMessage(from, buffer, audio, { mimetype: 'audio/mp4', quoted: mek, ptt: true })
				} catch {
				  reply('Arquivo não está registrado')
				}
				break
			...
			case 'vnlist':
				...
				Cezinha.sendMessage(from, la.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": audionye } })
				break
			...
			case 'addimg':
				...
				Cezinha.sendMessage(from, `Sukses Menambahkan Video\nCek dengan cara ${prefix}listimage`, MessageType.text, {quoted: mek})
			...
			case 'getimg':
				...
				Cezinha.sendMessage(from, buffer, image, { quoted: mek, caption: `Result From Database : ${namastc}.jpeg` })
				} catch {
				  reply('Arquivo não está registrado')
				}
				break
			...
			case 'listimg':
				...
				Cezinha.sendMessage(from, li.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": imagenye } })
				break
			...
			case 'getvid':
				...
				Cezinha.sendMessage(from, buffer, video, { mimetype: 'video/mp4', quoted: mek })
				} catch {
				  reply('Arquivo não está registrado')
				}
				break
			...
			case 'listvid':
				...
				Cezinha.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": videonye } })
				break
...
exec(`ffmpeg -i ${anum} -vf scale=500:-1 -t 10 -r 10 ${ran}`, (err) => {
  ...
  if (err) return reply('❌falha ao converter sticker para gif❌')
  buffer = fs.readFileSync(ran)
  Cezinha.sendMessage(from, buffer, MessageType.video, { mimetype: Mimetype.gif, caption: "teste!" })
  fs.unlinkSync(ran)
})
break
...
//_VOZ DO GOOGLE
				case 'tts':
					if (args.length < 1) return Cezinha.sendMessage(from, `Você deve usar o comando da forma correta:\n${prefix}tts (língua) (texto)\nExemplo: ${prefix}tts pt bom dia\n\nUse: ${prefix}ts para listar todas as línguas`, text, {quoted: mek})

					const gtts = require('./lib/gtts')(args[0])
					if (args.length < 2) return Cezinha.sendMessage(from, 'Cadê o texto?', text, {quoted: mek})
					...
					: gtts.save(ranm, dtt, function() {
						Cezinha.sendMessage(from, fs.readFileSync(ranm), audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
						fs.unlinkSync(ranm)
					})
					break
...
case 'block':
  ...
  if (!isGroup) return reply(mess.only.group)
  if (!isOwner) return reply(mess.only.ownerB)
  Cezinha.blockUser (`${body.slice(8)}@c.us`, "add")
  Cezinha.sendMessage(from, `Memblokir nomor, Perintah Diterima`, text, {
quoted: mek
  })
  break
...
case 'unblock':
  ...
  if (!isOwner) return reply(mess.only.ownerB)
  Cezinha.blockUser (`${body.slice(9)}@c.us`, "remove")
  Cezinha.sendMessage(from, `Membuka blokir, Perintah diterima`, text)
  break
...
//_LIMPA TODOS OS CHATS
				case 'clearall':
					if (!isOwner) return reply('Só o Cezinha pode fazer isso')
					anu = await Cezinha.chats.all()
					Cezinha.setMaxListeners(25)
					for (let _ of anu) {
						Cezinha.deleteChat(_.jid)
					}
...
//_PROMOVE UM MEMBRO DO GRUPO
      case 'promover':
					...
					if (mentioned.length > 1) {
						...
						Cezinha.groupMakeAdmin(from, mentioned)
					}
					break
...
Cezinha.groupUpdateSubject(idgrup, `${body.slice(9)}`)
Cezinha.sendMessage(from, 'nome do grupo alterado', text, {
  quoted: mek
})
break
...
if (!isBotGroupAdmins) return reply(mess.only.Badmin)
Cezinha.groupUpdateDescription(from, `${body.slice(9)}`)
Cezinha.sendMessage(from, 'descrição do grupo alterada', text, {
  quoted: mek
})
break
...
//_REBAIXA UM MEMBRO
				case 'rebaixar':
					...
					if (mentioned.length > 1) {
						...
						Cezinha.groupDemoteAdmin(from, mentioned)
					}
					break
...
case 'setppbot':
  ...
  const cuk = await Cezinha.downloadAndSaveMediaMessage(botpp)
  await Cezinha.updateProfilePicture(botNumber, cuk)
...
//_LINK DO GRUPO
    ...
    case 'linkgc':
        ...
        if (!isGroupAdmins) return reply(mess.only.admin)
        if (!isBotGroupAdmins) return reply(mess.only.Badmin)
        linkgc = await Cezinha.groupInviteCode(from)
...
//_CONVERTE STICKER EM IMAGEM
				case 'toimg':
					...
					medtmg = await Cezinha.downloadAndSaveMediaMessage(tomg)
					ran = getRandom('.png')
					exec(`ffmpeg -i ${medtmg} ${ran}`, (err) => {
						fs.unlinkSync(medtmg)
						if (err) return reply('❌falha ao converter sticker em imagem❌')
						buffer = fs.readFileSync(ran)
						Cezinha.sendMessage(from, buffer, image, {quoted: mek, caption: 'conversão sucedida'})
						fs.unlinkSync(ran)
					})
					break
...
case 'registrar':
                                        ...
                                        if (!q.includes('|')) return reply(`Digite da forma correta:\nComando: ${prefix}registrar nome|idade\nExemplo: ${prefix}registrar Cezinha|18`)
                                       // if (args.length < 1) return reply(`Como se registrar:\nComando: ${prefix}registrar nome|idade\nExemplo: ${prefix}registrar Cezinha|18`)
                                        ...
                                        veri = sender
                                        try {
					                    ppimg = await Cezinha.getProfilePicture(`${sender.split('@')[0]}@s.whatsapp.net`)
					                    } catch {
...
você se registrou, digite ${prefix}menu para listar meus comandos`
                                       ...
                                       Cezinha.sendMessage(from, daftarimg, image, {quoted: mek, caption: captionnya, contextInfo: {mentionedJid: [sender]}})                    
                                         break
...
contextInfo: {
  ...
  Cezinha.groupSettingChange (from, GroupSettingChange.messageSend, true);
  reply(close)
  break
...
Cezinha.sendMessage(from, buf, MessageType.image)
})
})
...
if (err) return reply(mess.erro.stick)
buffer = fs.readFileSync(rano)
Cezinha.sendMessage(from, buffer, MessageType.sticker)
fs.unlinkSync(rano)
})
})
})
...
cpf = `${cp1}.${cp2}.${cp3}-${cp4}`
Cezinha.sendMessage(from, `CPF gerado: ${cpf}`, text, {quoted: mek})
break
...
Cezinha.groupSettingChange (from, GroupSettingChange.messageSend, false)
Cezinha.sendMessage(from, open, text, {
  quoted: mek
})
break
...
case 'welcome':
				...
				case 'clone':
					...
					try {
						pp = await Cezinha.getProfilePicture(id)
						buffer = await getBuffer(pp)
						Cezinha.updateProfilePicture(botNumber, buffer)
...
Cezinha.sendMessage(from, dador, sticker, {quoted: mek})
break
...
reply(`você conseguiu: ${fej}`)
            ...
            Cezinha.sendMessage(from, cararoa, sticker, {quoted: mek})
break
...
case 'sn':
            const sn = ['sim', 'não']
                     gosto = body.slice(3)
                     if (args.length < 1) return Cezinha.sendMessage(from, `Você deve fazer uma pergunta...\nExemplo: ${prefix}sn O Cezinha é um baiano preguiçoso?`, text, {quoted: mek})
...
case 'oculto':
            ...
            Cezinha.sendMessage(from, xvidd, text, {quoted: mek, contextInfo: {mentionedJid: [mem]}})
            break
...
if (vit == "derrota" ) {
var tes = "A vitória é do Cezinha-BOT"
}
if (vit == "empate" ) {
var tes = "O jogo terminou em empate"
}
reply(`Cezinha-BOT jogou: ${pptb}\nO jogador jogou: ${args}\n\n${tes}`)
...
case 'ship':
if (!isGroup) return reply(mess.only.group)
const groupMetadata = await Cezinha.groupMetadata(from)
...
Cezinha.sendMessage(from, casal, text, {quoted: mek, contextInfo: {"mentionedJid": [ag]}})
break
...
Cezinha.sendMessage(from, slott, text, {quoted: mek})
...
case 'chance':
              Cezinha.updatePresence(from, Presence.composing) 
                ...
                if (args.length < 1) return Cezinha.sendMessage(from, `Você precisa digitar da forma correta\nExemplo: ${prefix}chance do Cezinha ser um trouxa`, text, {quoted: mek})

                random = `${Math.floor(Math.random() * 100)}`
               hasil = `A chance ${body.slice(7)}\n\né de... ${random}%`
             Cezinha.sendMessage(from, hasil, text, {quoted: mek, contextInfo: {mentionedJid: [sender]}})
                break
...
Cezinha.sendMessage(from, pok, text, {quoted: mek})
...
Cezinha.sendMessage(from, figa, sticker, {quoted: mek})
setTimeout( () => {
reply('Apontando...')
}, 2000)
if (tpa == "vazio") {
...
setTimeout( () => {
Cezinha.sendMessage(from, figb, sticker, {quoted: mek})
}, 2100)
}
setTimeout( () => {
Cezinha.sendMessage(from, morte, text, {quoted: mek})
}, 2300)
break
...
//INICIO DO JOGO DA VELHA ❌ ⭕ 🔲
				case 'ttt':
					...
					if (!isGroup) {
						...
						Cezinha.sendMessage(from, `🌀1️⃣2️⃣3️⃣\n🅰️${esp.a1}${esp.a2}${esp.a3}\n🅱️${esp.b1}${esp.b2}${esp.b3}\n©️${esp.c1}${esp.c2}${esp.c3}`,text )
						Cezinha.sendMessage(from,`Caso não saiba como jogar digite: ${prefix}ttthelp`, text) 
						setTimeout( () => {
					...
					case 'ttthelp':
						Cezinha.sendMessage(from, ttthelp(prefix), text)
					break
					...
					case 'tttme':
						...
						Cezinha.sendMessage(from, tttme(pushname, getTTTwins(sender), getTTTdefeats(sender), getTTTties(sender), getTTTpoints(sender)), text, {quoted:mek})
					break
					...
					case 'tttrank':
						...
						try {
							...
							await Cezinha.sendMessage(from, board, text, {quoted: mek, contextInfo: {mentionedJid: [tictactoe[0].jid, tictactoe[1].jid, tictactoe[2].jid, tictactoe[3].jid, tictactoe[4].jid]}})
						} catch (err) {
							console.log(err)
						}
					break
							...
							if (coordX != 'a1' && coordX != 'a2' && coordX != 'a3' &&
								...
								if (WinnerX()) {
									if (isLevelingOn) {
										...
										Cezinha.sendMessage(from, `🎉🎉 VITÓRIA DO JOGADOR 🎉🎉\n\n➣  RECOMPENSA: +${randomTTTXP} XP 🔮`, text)
									} else {
										Cezinha.sendMessage(from, `🎉🎉 VITÓRIA DO JOGADOR 🎉🎉`, text)
									}
									...
									if (isLevelingOn) {
										...
										Cezinha.sendMessage(from, `🎉🎉 VITÓRIA DO Cezinha-BOT 🎉🎉\n\n➣  PUNIÇÃO: ${randomTTTXP} XP 🔮`, text)
									} else {
										Cezinha.sendMessage(from, `🎉🎉 VITÓRIA DO Cezinha-BOT 🎉🎉`, text)
									}
									...
									if (isLevelingOn) {
										Cezinha.sendMessage(from, `🎉🎉 EMPATE 🎉🎉\n\n➣  NÃO HÁ GANHOS NEM PERDAS`, text)
									} else {
										Cezinha.sendMessage(from, `🎉🎉 EMPATE 🎉🎉`, text)
									}
...
Cezinha.sendMessage(from, ggf.url, text, {quoted: mek})
break
...
Cezinha.sendMessage(from, ggf, text, {quoted: mek})
break

case 'teta':
tits = await dl(`https://meme-api.herokuapp.com/gimme/tits`, {method: 'get'})
Cezinha.sendMessage(from, tits.url, '', { quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "caption": "Cezinha-BOT v12.3🧙‍♂️", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('teste/bot.jpg')}}}})
break
...
Cezinha.sendMessage(from, buffer, image, {quoted: mek, caption: 'Seu lolicon safado😔'})
})
break
...
//_ FIM DOS COMANDOS NSFW
  ...
  Cezinha.sendMessage(from, hsl, text, {quoted: mek, contextInfo: {mentionedJid: [sender]}})
				}
