...
Cezinha.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				Cezinha.sendMessage(mdata.id, welt, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
...
setTimeout( () => {
Cezinha.groupRemove(from, [kic]).catch((e)=>{reply(`*ERR:* ${e}`)})
}, 1000)
setTimeout( () => {
Cezinha.updatePresence(from, Presence.composing)
reply("adios")
...
Cezinha.sendMessage(from, mz(prefix), text, tescuk, cr)
break
					...
					case 'lb':
						...
						try {
					     for (let i = 0; i < 3; i++) {
							...
							console.log(err)
							Cezinha.sendMessage(from, `É necessário que no mínimo 3 pessoas tenham level...`, text, {quoted: mek})
						}
					break
...
const bug = body.slice(10)
 if (args.length > 300) return Cezinha.sendMessage(from, 'Máximo 300 caracteres', msgType.text, {quoted: mek})
...
Cezinha.sendMessage('557499510904@s.whatsapp.net', options, text, {quoted: mek})
...
Cezinha.sendMessage(from, `${anu.success} 🐤`, text, {quoted: mek})
} catch {
reply(ptbr.erro)
}
break
...
Cezinha.sendMessage(from, `https://github.com/italuH/Cezinha-BOT` , text, {quoted: mek})
break
...
Cezinha.sendMessage(nomor+'@s.whatsapp.net', pesan, text)
break
...
case 'italu':
case 'italo':
await Cezinha.sendMessage(from, {displayname: "Italu🧙‍♂️", vcard: vcard}, MessageType.contact)
break
...
Cezinha.sendMessage(from, options, text, { quoted: mek } )
break
...
Cezinha.sendMessage(from, buf, image, {quoted: mek,caption: "🐱"})
})})
} catch (e) {
console.log(`Error :`, color(e,'red'))
reply('❌ocorreu um erro❌\n\nTente novamente. ')
...
Cezinha.sendMessage(from, buf, MessageType.image, {quoted: mek})
})
})
} catch (e) {
...
reply('A mimir...')
exec(pm2, (err, stdout) => {
if(err) return Cezinha.sendMessage(from, "Erro", text, {quoted: mek})
if (stdout) {
...
const mediaaa = await Cezinha.downloadAndSaveMediaMessage(encmediaaa)
reply(ptbr.wait())
a = await webp2gifFile(mediaaa)
mp4 = await getBuffer(a.result)
Cezinha.sendMessage(from, mp4, MessageType.video, {mimetype: 'video/gif', filename: `stick.gif`, quoted: mek, caption: '✅'})
...
Cezinha.sendMessage(from, buffer, image, {caption: inf, contextInfo: {mentionedJid: [me]}})
break
...
` 
  ...
  ‣Descrição:
  ${desc ? desc : ''}`
await Cezinha.sendMessage(from, buffer, image, {quoted: mek, caption: infogp, contextInfo: {mentionedJid: [owner.split]}})
break
...
//aanu = await fetchJson(`https://api-Cezinha.italuh.repl.co/api/yta?url=${anu.result.source}`)
...
try {
anu = await fetchJson(`https://api-Cezinha.italuh.repl.co/api/yta?url=${play}`)
...
.on('end', function () {
console.log('Finish')
exec(`webpmux -set exif ${addMetadata('Cezinha-BOT', 'Italu')} ${ran} -o ${ran}`, async (error) => {
...
fs.unlinkSync(ranp)
if (err) return reply(ptbr.stick())
exec(`webpmux -set exif ${addMetadata('Cezinha-BOT', 'Italu')} ${ranw} -o ${ranw}`, async (error) => {
if (error) return reply(ptbr.stick())
...
.on('error', function (err) {
console.log(`Error : ${err}`)
exec(`webpmux -set exif ${addMetadata('Cezinha-BOT', 'Italu')} ${rano} -o ${rano}`, async (error) => {
fs.unlinkSync(media)
reply(ptbr.stick())
})
})
...
case 'ship':
if (!isGroup) return reply(ptbr.group())
const ag = await Cezinha.getgroupMetadata.participants(from)
...
console.log(err)
await Cezinha.sendMessage(from, `Humm, é necessário que no mínimo 3 pessoas tenham jogado...`, text, {quoted: mek})
}
break
...
axios.get(`https://nekos.life/api/v2/img/kemonomimi`).then((res)=>{
imageToBase64(res.data.url).then((response) => {var buf = Buffer.from(response, 'base64');
Cezinha.sendMessage(from, buf, image, {quoted: mek})
})})
} catch (e) {
console.log(`Error :`, color(e,'red'))
reply('Ocorreu um erro')
...
(ress) => {
var buf = Buffer.from(ress, 'base64')
Cezinha.sendMessage(from, buf, MessageType.video, {mimetype: 'video/gif', filename: `stick.gif`, quoted: mek, caption: '✅'})
})
})
} catch (e) {
console.log(`Error :`, color(e,'red'))
